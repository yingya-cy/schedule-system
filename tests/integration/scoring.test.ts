...
describe('Template CRUD', () => {
  ...
  afterAll(async () => {
    await supertest(app).delete(`/api/scoring/templates/${templateId}`);
  });
});
...
describe('Competition and Contestants', () => {
  ...
  beforeAll(async () => {
    const tRes = await supertest(app)
      .post('/api/scoring/templates')
      .send({
        name: '比赛测试模板',
        total_score: 50,
  ...
  afterAll(async () => {
    await supertest(app).delete(`/api/scoring/competitions/${competitionId}`);
    await supertest(app).delete(`/api/scoring/templates/${templateId}`);
  });
});
